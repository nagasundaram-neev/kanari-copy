Feature: Update Outlet Type
    ...
    Scenario: Successfully update user account
      ...
      When I send a PUT request to "/api/outlet_types/2" with the following:
      """
      {
        "outlet_type" : {
      ...
      null
      """
      And an outlet type with name "Very Casual Dining" should exist
    ...
    Scenario: Outlet type doesn't exist
      ...
      And the following outlet types exist
        ...
        |Casual Dining      | 2   |
      When I authenticate as the user "auth_token_123" with the password "random string"
      And I send a PUT request to "/api/outlet_types/3" with the following:
      """
      {
        "outlet_type" : {
          "name": "Very Casual Dining"
    ...
    Scenario Outline: User's role is not kanari_admin
      ...
      And I send a PUT request to "/api/outlet_types/2" with the following:
      """
      {
        "outlet_type" : {
